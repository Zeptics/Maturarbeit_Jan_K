...
source_directory = os.path.join(os.getcwd(), 'Tensorflow', 'workspace', 'models', 'ssd_mobilenet_v2_320x320_coco17_tpu-8', 'v2')
# destination_root_directory = r'E:\faster_rcnn_resnet152_v1_640x640_coco17_tpu-8\v1\checkpoints_steps'
destination_root_directory = os.path.join('Tensorflow', 'workspace', 'models', 'ssd_mobilenet_v2_320x320_coco17_tpu-8', 'v2', 'checkpoints_steps')
total_steps = 100000
...
while True:
    ...
    for file in new_checkpoints:
        ...
        if file.endswith('.index'):
            ...
            print(num_steps, (total_steps/100) + 1)
            if int(num_steps) == (total_steps/100) + 1:
            ...
            elif int(num_steps) <= 10:
                destination_excact_dir = os.path.join(destination_root_directory, f'step{int(num_steps) * 100}')
                os.makedirs(destination_excact_dir, exist_ok=True)
                data_file_name = f'{file.split(".")[0]}.data-00000-of-00001'
                copy_files(file, data_file_name, destination_excact_dir)
            # Until 5000 steps, save a model for every 500 steps
            elif int(num_steps) <= 50:
                left_over = int(num_steps) % 5
                if left_over != 0:
                    exactly_wanted_ckpt_file = False
                    step_folder_num = int(num_steps) + (5 - left_over)
                else:
                    exactly_wanted_ckpt_file = True
                    step_folder_num = int(num_steps)
                destination_excact_dir = os.path.join(destination_root_directory, f'step{int(step_folder_num) * 100}')
                os.makedirs(destination_excact_dir, exist_ok=True)
                data_file_name = f'{file.split(".")[0]}.data-00000-of-00001'
                if exactly_wanted_ckpt_file:
                    copy_files(file, data_file_name, destination_excact_dir)
                # Until 20000 steps save a model for every 1000 steps
            elif int(num_steps) <= 200:
                left_over = int(num_steps) % 10
                if left_over != 0:
                    exactly_wanted_ckpt_file = False
                    step_folder_num = int(num_steps) + (10 - left_over)
                else:
                    exactly_wanted_ckpt_file = True
                    step_folder_num = int(num_steps)
                destination_excact_dir = os.path.join(destination_root_directory, f'step{int(step_folder_num) * 100}')
                ...
                left_over = int(num_steps) % 50
                if left_over != 0:
                    exactly_wanted_ckpt_file = False
                    step_folder_num = int(num_steps) + (50 - left_over)
                else:
                    exactly_wanted_ckpt_file = True
                    step_folder_num = int(num_steps)
                destination_excact_dir = os.path.join(destination_root_directory, f'step{int(step_folder_num) * 100}')
